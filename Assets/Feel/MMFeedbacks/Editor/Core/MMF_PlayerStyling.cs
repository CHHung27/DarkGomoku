    ...
    public static class MMF_PlayerStyling
    {
        ...
        static public Rect DrawSimpleHeader(ref bool expanded, ref bool activeField, string title)
        {
            ...
            if (
                ...
                && _labelRect.Contains(e.mousePosition)
                && e.button == 0
            )
            {
                expanded = !expanded;
        ...
        {
            float thisTime =
                timing.TimescaleMode == TimescaleModes.Scaled ? Time.time : Time.unscaledTime;
            float thisDeltaTime =
                timing.TimescaleMode == TimescaleModes.Scaled
                    ? Time.deltaTime
                    : Time.unscaledDeltaTime;
            if (host.ForceTimescaleMode)
            {
                thisTime =
                    host.ForcedTimescaleMode == TimescaleModes.Scaled
                        ? Time.time
                        : Time.unscaledTime;
                thisDeltaTime =
                    host.ForcedTimescaleMode == TimescaleModes.Scaled
            ...
            if (
                timing.MMFeedbacksDirectionCondition
            ...
            if (host.DisplayFullDurationDetails)
            {
                ...
                {
                    float delayBetweenRepeats = host.ApplyTimeMultiplier(
            ...
            if (
                host.IsPlaying && (startedAt > 0f) && (thisTime - startedAt < totalDuration + 0.05f)
            )
            {
